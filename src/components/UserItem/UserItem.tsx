...
interface Props {
	...
	ul: React.MutableRefObject<HTMLUListElement | null>;
}

const UserItem = ({ user, ul }: Props) => {
	const mainLi = useRef<HTMLLIElement>(null);
	const deleterSpan = useRef<HTMLElement>(null);
	...
	useEffect(() => {
		...
		deleterSpan.current?.addEventListener("click", () => {
			if (window.confirm(`Are you sure you want to delete "${user.login}" from list??`)) {
				let arrayOfLi;
				if (ul.current) {
					arrayOfLi = [...ul.current.querySelectorAll("li")];
